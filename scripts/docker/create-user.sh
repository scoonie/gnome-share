...
if ! getent group gnome-share-group > /dev/null 2>&1; then
    addgroup -g "$PGID" gnome-share-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u gnome-share > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G gnome-share-group gnome-share > /dev/null 2>&1
    else
